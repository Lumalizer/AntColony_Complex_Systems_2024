...
cor <- ccf(a1$angle, a1$speed, lag.max=6,type="correlation",plot=FALSE)$acf
cmap <- CCM(dataFrame = a1, E = 8, Tp=0,columns = "speed", target = "angle", libSizes = "10 343", sample = 100, showPlot =
              TRUE)

abline(h = max(abs(cor)), col = "black",lty=2)
